...
use Arifur\BookstackBackup\Http\Controllers\BackupSettingsController;
...
Route::middleware(['web', 'auth', 'can:settings-manage'])->group(function () {
    ...
    Route::post('/settings/backups', [BackupSettingsController::class, 'updateBackupSettings'])->name('backups.settings.update');

    Route::get('/settings/backups/schedule', [BackupController::class, 'schedule'])->name('backups.schedule');
    Route::post('/settings/backups/schedule', [BackupSettingsController::class, 'updateScheduleSettings'])->name('backups.schedule.update');

    Route::get('/settings/backups/backup-settings', [BackupController::class, 'backupSettings'])->name('backups.backup-settings');
    Route::post('/settings/backups/backup-settings', [BackupSettingsController::class, 'updateBackupSettingsSection'])->name('backups.backup-settings.update');

    Route::get('/settings/backups/remote', [BackupController::class, 'remote'])->name('backups.remote');
    Route::post('/settings/backups/remote', [BackupSettingsController::class, 'updateRemoteSettings'])->name('backups.remote.update');

    Route::post('/settings/backups/create', [BackupController::class, 'create'])->name('backups.create');
    Route::get('/settings/backups/create-progress/{token}', [BackupController::class, 'createProgress'])->name('backups.create-progress');
